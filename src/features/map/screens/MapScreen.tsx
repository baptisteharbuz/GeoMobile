...
export default function MapScreen({ style, latitude, longitude }: Readonly<MapScreenProps>) {
    ...
    const handleMarkerDragEnd = (markerId: string, coordinate: number[]) => {
        ...
        const updated = markers.map((m) => (m.id === markerId ? { ...m, latitude, longitude } : m));
        setMarkers(updated);
        saveMarkers(updated);
    ...
    const handleMapPress = (event: any) => {
        let coords: [number, number] | undefined =
            event?.geometry?.coordinates || event?.features?.[0]?.geometry?.coordinates || event?.coordinates;
    ...
    return (
        <View style={Mapstyles.container}>
            ...
            >
                <Mapbox.Camera ref={cameraRef} centerCoordinate={coordinates} zoomLevel={13} />
                        ...
                        >
                            <Animated.View
                                style={[
                                    ...
                                    markerAnimations[marker.id]
                                        ? { transform: [{ scale: markerAnimations[marker.id] }] }
                                        : undefined,
                                ]}
                            >
