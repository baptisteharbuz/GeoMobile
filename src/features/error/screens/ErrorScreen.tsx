...
export default function ErrorScreen({ onRetry }: Readonly<ErrorScreenProps>) {
    ...
    return (
        <View style={[ErrorStyles.container, { paddingTop: insets.top + 20 }]}>
            ...
            <Text style={ErrorStyles.instructions}>Pour activer la localisation :</Text>
            ...
            <View style={ErrorStyles.buttonContainer}>
                <Button title="📍 Réessayer" onPress={onRetry} variant="primary" />
            </View>

            <View style={ErrorStyles.buttonContainer}>
                <Button title="⚙️ Ouvrir les paramètres" onPress={openSettings} variant="secondary" />
            </View>
        </View>
    );
}
